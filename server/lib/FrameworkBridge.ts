...
export class FrameworkBridge {
        ...
        try {
            ...
            if (exports['qb-core']?.GetCoreObject) {
                ...
                return {
                    ...
                    source: src,
                    phone,
                    getMoney: (type: 'bank' | 'cash') => player.Functions?.GetMoney ? player.Functions.GetMoney(type) : (player.PlayerData?.money?.[type] ?? 0),
                    removeMoney: (type: 'bank' | 'cash', amount: number) => player.Functions?.RemoveMoney ? player.Functions.RemoveMoney(type, amount) : false,
                    setMeta: (key: string, value: any) => {
                        if (player.Functions?.SetMetaData) {
                            player.Functions.SetMetaData(key, value);
                        } else if (player.PlayerData?.metadata) {
    ...
    public static removeInventoryItem(src: number, player: any, item: string, count: number): boolean {
