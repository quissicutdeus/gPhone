...
describe("messages store", () => {
    ...
    it("loads messages for a conversation and assigns sender tags", async () => {
        await messagesStore.loadMessages(2);
        const msgs = get(messagesStore.messages)[2];
    ...
    it("sends message, updates conversation snippet, and sorts conversation to top", async () => {
        ...
        expect(get(messagesStore)[0].id).toBe(2);

        // Reply to Ursula (id 1)
        await messagesStore.sendMessage(1, "Fine I am calling now");

        const updatedConvs = get(messagesStore);
    ...
    it("marks conversation as read", async () => {
        await messagesStore.loadConversations();
        expect(get(messagesStore)[1].unreadCount).toBe(25);

        await messagesStore.markAsRead(1);
        expect(get(messagesStore)[1].unreadCount).toBe(0);
    });
});
