...
test.describe('Blabber', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // inside the add-on's iframe — see `dismissToasts`.
  ...
  test('asks a brand-new player for a handle before showing a feed', async ({ page }) => {
    ...
    await expect(frame.locator('text=Pick a handle')).toBeVisible();
    ...
    await expect(frame.getByRole('button', { name: 'Blab', exact: true })).toHaveCount(0);

    await frame.locator('input[placeholder="handle"]').fill('newcomer');
    await frame.getByRole('button', { name: 'Claim', exact: true }).click();

    // Claiming is what reveals the feed, and the new handle is who you post as.
    await expect(frame.locator('text=traffic on the interstate')).toBeVisible();
    await expect(frame.getByRole('button', { name: 'Posting as @newcomer' })).toBeVisible();
  });
  ...
  const openComposer = (page: import('@playwright/test').Page) =>
    addOnFrame(page, 'blabber').getByRole('button', { name: 'Blab', exact: true }).click();

  test('posts a Blab and shows it at the top', async ({ page }) => {
    const frame = addOnFrame(page, 'blabber');
    await openComposer(page);
    await frame.locator('textarea').fill('e2e was here');
  ...
  test('closes the composer on Back rather than leaving the app', async ({ page }) => {
     ...
     * while a text field has focus. The key itself goes to `page`: frame keys are forwarded to
     * the shell, which owns the level stack.
     */
    const frame = addOnFrame(page, 'blabber');
    await openComposer(page);
  ...
  test('opens a profile and switches between Blabs and Replies', async ({ page }) => {
    ...
    await expect(frame.getByRole('button', { name: 'Blabs', exact: true })).toBeVisible();
    await expect(frame.getByRole('button', { name: 'Replies', exact: true })).toBeVisible();

    await frame.getByRole('button', { name: 'Replies', exact: true }).click();
    ...
    await frame.getByRole('button', { name: 'Blabs', exact: true }).click();
    await expect(frame.locator('text=first')).toBeVisible();
  });
  ...
  test("opens a profile from a Blab's avatar, not only from the name", async ({ page }) => {
    ...
    await expect(frame.getByRole('button', { name: 'Blabs', exact: true })).toBeVisible();
    await expect(frame.getByRole('button', { name: 'Message @nightowl' })).toBeVisible();
  });

  test('renders markup in a post as text rather than interpreting it', async ({ page }) => {
    const frame = addOnFrame(page, 'blabber');
    await openComposer(page);
    await frame.locator('textarea').fill('<b>not bold</b>');
    // Role-based and exact: Blabber's own Store description begins "Post short updates...",
    // and the backgrounded Store app is still in the DOM. `inert` keeps it out of the
    // accessibility tree, which a text match does not respect.
    await frame.getByRole('button', { name: 'Post', exact: true }).click();
    ...
    await expect(frame.locator('text=<b>not bold</b>')).toBeVisible();
    await expect(frame.locator('article b')).toHaveCount(0);
  });

  test('ears a Blab and fills the heart', async ({ page }) => {
    const frame = addOnFrame(page, 'blabber');
    const first = frame.locator('article').first();
  ...
  test('mouths a Blab and shows it repeated at the top', async ({ page }) => {
    const frame = addOnFrame(page, 'blabber');
    await frame.locator('article').first().getByRole('button', { name: 'Mouth' }).click();

    await expect(frame.locator('article').first().locator('text=Mouthed')).toBeVisible();
  });
  ...
  test('opens a Blab and replies to a reply, both landing in the same flattened view', async ({
    ...
    await expect(frame.locator('text=congratulations on being first')).toBeVisible();
    await expect(frame.locator('text=thank you')).toBeVisible();
    ...
    await frame
      ...
      .click();
    await expect(frame.getByPlaceholder('Reply to @nightowl')).toBeVisible();

    await frame.locator('textarea').fill('replying to the reply');
    await frame.getByRole('button', { name: 'Post', exact: true }).click();

    await expect(frame.locator('text=replying to the reply')).toBeVisible();
    // Still one screen — no second Thread/BlabDetail level was pushed.
    await expect(frame.locator('h1', { hasText: 'Blab' })).toBeVisible();
  });
  ...
  test('opens a DM thread from the inbox and replies', async ({ page }) => {
    ...
    await expect(frame.locator('text=saw your post')).toBeVisible();
    await frame.locator('button', { hasText: 'nightowl' }).click();

    await frame.locator('textarea').fill('thanks!');
    ...
    await expect(frame.locator('text=thanks!')).toBeVisible();
  });
  ...
  const openIdentityMenu = (page: import('@playwright/test').Page) =>
    addOnFrame(page, 'blabber')
      .getByRole('button', { name: /^Posting as @/ })
      .click();
  ...
  test('switches the posting identity from the header menu', async ({ page }) => {
    ...
    await expect(frame.locator('text=Posting as · 2 of 3')).toBeVisible();

    await frame.locator('button', { hasText: 'ada_alt' }).click();

    // The header avatar names whoever a post would go out as, so it is the proof the switch took.
    await expect(frame.getByRole('button', { name: 'Posting as @ada_alt' })).toBeVisible();
  });
  ...
  test('dismisses the identity menu by clicking away from it', async ({ page }) => {
    ...
    await openIdentityMenu(page);
    await expect(frame.getByRole('button', { name: 'Edit profile' })).toBeVisible();

    await frame.getByRole('button', { name: 'Close menu' }).click();

    await expect(frame.getByRole('button', { name: 'Edit profile' })).toHaveCount(0);
  });
  ...
  test('edits a profile, which had no UI at all', async ({ page }) => {
    ...
    await expect(frame.locator('text=a handle is claimed once')).toBeVisible();

    await frame.locator('#blabber-display-name').fill('Ada L');
    await frame.locator('#blabber-bio').fill('counting machines');
    await frame.getByRole('button', { name: 'Save', exact: true }).click();

    // A `useAppAction` success toast is raised through the host and rendered by the
    // shell's `ToastHost`, in the shell's document — never inside the add-on's frame.
  ...
  test('claims a second handle, which the UI used to make unreachable', async ({ page }) => {
    const frame = addOnFrame(page, 'blabber');
    await openIdentityMenu(page);
    await frame.getByRole('button', { name: 'Claim another handle' }).click();

    await frame.locator('input[placeholder="handle"]').fill('e2e_alt');
    await frame.getByRole('button', { name: 'Claim', exact: true }).click();
    ...
    await expect(frame.getByRole('button', { name: 'Posting as @e2e_alt' })).toBeVisible();
  });
  ...
  test('backs out of claiming a handle without one', async ({ page }) => {
    ...
    await expect(frame.locator('text=Pick a handle')).toHaveCount(0);
    await expect(frame.getByRole('button', { name: 'Posting as @ada' })).toBeVisible();
  });
  ...
  test('starts a DM from a profile, before any thread exists', async ({ page }) => {
    ...
    await frame.getByRole('button', { name: 'Message @nightowl' }).click();
    ...
    await expect(frame.locator('h1', { hasText: 'Night Owl' })).toBeVisible();

    await frame.locator('textarea').fill('hello from a profile');
    await frame.getByRole('button', { name: 'Send', exact: true }).click();

    await expect(frame.locator('text=hello from a profile')).toBeVisible();
  });
  ...
  test.describe('Following', () => {
    const followNightowl = async (page: import('@playwright/test').Page) => {
      ...
      await expect(frame.getByRole('button', { name: 'Following', exact: true })).toBeVisible();
    };

    test('starts empty, and says which kind of empty it is', async ({ page }) => {
      const frame = addOnFrame(page, 'blabber');
      await frame.getByRole('button', { name: 'Following' }).click();

      await expect(frame.locator('text=Nothing from anyone yet')).toBeVisible();
      // Not "nobody has posted" — nobody has been followed, which is a different statement and
      // the one the player can act on.
      await expect(frame.locator('text=Follow somebody from their profile')).toBeVisible();
    });
    ...
    test('a followed account’s Blabs turn up in the tab', async ({ page }) => {
      ...
      await expect(frame.locator('text=anyone up?')).toBeVisible();
      // Only followed accounts: @ada is the player's own and was never followed.
      await expect(frame.locator('text=traffic on the interstate')).toHaveCount(0);
    });

    test('unfollowing empties the tab again', async ({ page }) => {
      const frame = addOnFrame(page, 'blabber');
      await followNightowl(page);
      await frame.getByRole('button', { name: 'Following', exact: true }).click();
    ...
    test('the follower count moves with the button', async ({ page }) => {
      ...
      await expect(frame.locator('text=0 followers')).toBeVisible();
      await frame.getByRole('button', { name: 'Follow', exact: true }).click();
      await expect(frame.locator('text=1 followers')).toBeVisible();
    });
    ...
    test('a follower count opens the list of who they are', async ({ page }) => {
      ...
      await frame.getByRole('button', { name: /followers$/ }).click();
      await expect(frame.locator('h1', { hasText: 'Followers' })).toBeVisible();
      await expect(frame.locator('text=No followers yet')).toBeVisible();
      await expect(frame.locator('text=Nobody follows @nightowl yet')).toBeVisible();
      ...
      await page.keyboard.press('Backspace');
      await expect(frame.locator('h1', { hasText: 'Profile' })).toBeVisible();
      await expect(frame.getByRole('button', { name: 'Follow', exact: true })).toBeVisible();
    });
    ...
    test('following somebody puts you in their followers and them in your following', async ({
      ...
      await frame.getByRole('button', { name: /followers$/ }).click();
      await expect(frame.locator('h1', { hasText: 'Followers' })).toBeVisible();
      await expect(frame.locator('text=@ada')).toBeVisible();
      ...
      await frame.getByRole('button', { name: /Ada/ }).first().click();
      await expect(frame.locator('h1', { hasText: 'Profile' })).toBeVisible();
      // Your own profile, so no Follow button on it.
      await expect(frame.getByRole('button', { name: 'Follow', exact: true })).toHaveCount(0);

      // The other direction of the same relation, off the other end of the same table.
      await frame.getByRole('button', { name: /following$/ }).click();
      await expect(frame.locator('h1', { hasText: 'Following' })).toBeVisible();
      await expect(frame.locator('text=Night Owl')).toBeVisible();
    });
    ...
    test('an account following nobody says so, rather than showing an empty list', async ({
      ...
      await expect(frame.locator('text=Not following anyone')).toBeVisible();
      await expect(frame.locator('text=@ada has not followed anybody yet')).toBeVisible();
    });
    ...
    test('no Follow button on your own profile', async ({ page }) => {
      ...
      await expect(frame.getByRole('button', { name: 'Follow', exact: true })).toHaveCount(0);
      await expect(frame.getByRole('button', { name: /^Message @/ })).toHaveCount(0);
    });
  });
  ...
  test('a pushed mention raises a toast and moves the badge', async ({ page }) => {
    ...
    // The push and the toast it raises are both shell-side, so this stays on `page` throughout.
    await page.evaluate(() =>
      window.pushAppEvent?.(
        'blabber',
        'mention',
  ...
  test.describe('Notifications tab', () => {
    ...
    test('a notification opens the Blab it names, root included', async ({ page }) => {
      const frame = addOnFrame(page, 'blabber');
      await openTab(page);
      await frame.locator('text=anyone up? @ada').click();
      ...
      // with a leaf post.
      await expect(frame.getByRole('button', { name: "Night Owl's profile" })).toBeVisible();
    });
    ...
    test('Back leaves Notifications for the feed before leaving the app', async ({ page }) => {
      ...
      await openTab(page);
      await expect(frame.locator('h1', { hasText: 'Notifications' })).toBeVisible();
  ...
  test.describe('Search tab', () => {
    const openSearch = (page: import('@playwright/test').Page) =>
      addOnFrame(page, 'blabber').getByRole('button', { name: 'Search', exact: true }).click();

    test('a query under 2 characters fires no request', async ({ page }) => {
      const frame = addOnFrame(page, 'blabber');
      await openSearch(page);
      await frame.locator('input[placeholder="Search Blabber"]').fill('a');

      // Trending, not results — the empty/results branch never activates below 2 characters, so
      // neither empty state can be showing.
      await expect(frame.locator('text=Nothing trending yet')).toHaveCount(0);
    ...
    test('trending chips render before typing and disappear once a query is entered', async ({
      ...
      const frame = addOnFrame(page, 'blabber');
      await openSearch(page);
      await expect(frame.locator('button', { hasText: '#losangeles' })).toBeVisible();

      await frame.locator('input[placeholder="Search Blabber"]').fill('ad');
     ...
     *
     * `accountResults` paged `{ service: 'accounts' }` until GPHONE-16 step 4's review:
    ...
    test('finding a Blab by body and opening it lands on the flattened view', async ({ page }) => {
      ...
      await openSearch(page);
      await frame.getByRole('button', { name: 'Blabs', exact: true }).click();
      await frame.locator('input[placeholder="Search Blabber"]').fill('congratulations');
      ...
      await expect(frame.getByRole('button', { name: "Ada's profile" }).first()).toBeVisible();
    });
    ...
    test('tapping a tag inline, and the same tag from the Tags segment, both land on the same tag screen', async ({
      ...
      const frame = addOnFrame(page, 'blabber');
      await frame.locator('button', { hasText: '#losangeles' }).first().click();
      await expect(frame.locator('h1', { hasText: '#losangeles' })).toBeVisible();
      await page.keyboard.press('Backspace');

      await openSearch(page);
      await frame.getByRole('button', { name: 'Tags', exact: true }).click();
      await frame.locator('input[placeholder="Search Blabber"]').fill('los');
      await frame.locator('button', { hasText: '#losangeles' }).click();
